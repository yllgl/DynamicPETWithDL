...
def MyCumtrapz(y,x,dim=0,initial=None):
    ...
    return out
def one_iter(loss_fn,batch,t_all,decay,scant,net,project_net,kparam_net):
...
def train_net(loss_name,net,project_net,kparam_net,dataloader,val_dataloader,criterion,history:History,decay=float(np.log(2))/109.8,lr=1e-4,epochs=200,iter_output=100,split_image_size=(16,16),retrain=False,retrain_lr=False,load_path=None):
    ...
    minval = float("inf")
    writer = SummaryWriter(log_dir="log_version4_compare", filename_suffix=str(datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S')))
    if retrain:
        if os.path.exists("best_weights/net_version4_compare.pkl"):
            net.load_state_dict(torch.load("best_weights/net_version4_compare.pkl"))
            project_net.load_state_dict(torch.load("best_weights/project_net_version4_compare.pkl"))
            kparam_net.load_state_dict(torch.load("best_weights/kparam_net_version4_compare.pkl"))
            optimizer_net.load_state_dict(torch.load("best_weights/optimizer_net_version4_compare.pkl"))
            optimizer_project_net.load_state_dict(torch.load("best_weights/optimizer_project_net_version4_compare.pkl"))
            optimizer_kparam_net.load_state_dict(torch.load("best_weights/optimizer_kparam_net_version4_compare.pkl"))
            minval = torch.load("best_weights/minval_version4_compare.pkl")
    ...
    for epoch in range(epochs):
        ...
        for it,batch in enumerate(dataloader):
            ...
            loss = one_iter(loss_fn,batch,t_all,decay,scant,net,project_net,kparam_net)
            ...
            if it>1000 and it%500==0 and train_minval>loss_record.val():
                ...
                torch.save(net.state_dict(),f"train_process_weights/net_version4_compare_epoch{epoch}_it{it}_loss{train_minval}.pkl")
                torch.save(project_net.state_dict(),f"train_process_weights/project_net_version4_compare_epoch{epoch}_it{it}_loss{train_minval}.pkl")
                torch.save(kparam_net.state_dict(),f"train_process_weights/kparam_net_version4_compare_epoch{epoch}_it{it}_loss{train_minval}.pkl")
        ...
        if minval>history.history_dict["val_"+loss_name].avg:
            torch.save(net.state_dict(),"best_weights/net_version4_compare.pkl")
            torch.save(project_net.state_dict(),"best_weights/project_net_version4_compare.pkl")
            torch.save(kparam_net.state_dict(),"best_weights/kparam_net_version4_compare.pkl")
            torch.save(optimizer_net.state_dict(),"best_weights/optimizer_net_version4_compare.pkl")
            torch.save(optimizer_project_net.state_dict(),"best_weights/optimizer_project_net_version4_compare.pkl")
            torch.save(optimizer_kparam_net.state_dict(),"best_weights/optimizer_kparam_net_version4_compare.pkl")
            minval = history.history_dict["val_"+loss_name].avg
            torch.save(minval,"best_weights/minval_version4_compare.pkl")
...
if __name__=="__main__":
    ...
    kparam_net = nn.Sequential(nn.Conv2d(256,128,1),nn.GroupNorm(128//16,128),nn.LeakyReLU(),nn.Conv2d(128,128,1),nn.GroupNorm(128//16,128),nn.LeakyReLU(),nn.Conv2d(128,64,1),nn.GroupNorm(64//16,64),nn.LeakyReLU(),nn.Conv2d(64,7,1))
    project_net = project_net.cuda().float()
    kparam_net = kparam_net.cuda().float()

    train_net("loss",net,project_net,kparam_net,DataLoader(Subset(dataset,train_idx),batch_size=1,shuffle=True),DataLoader(Subset(dataset,val_idx),batch_size=1,shuffle=False),criterion,history,lr=0.5e-4,iter_output=1,load_path={"net":"best_weights/net_version4_compare.pkl","project_net":"best_weights/project_net_version4_compare.pkl","kparam_net":"best_weights/kparam_net_version4_compare.pkl"})